...
// up to MAX_SEED_LENGTH (40) + a random suffix ("-" + 8 hex = 9) tops out
// at 49, comfortably under 63. Raised from an original 20 — too short for
// the actual patterns this feature exists to support (the spec's own
// example, "happybirthday-from{{user_name}}-to{{recepient_name}}",
// resolves to 31+ characters with realistic names, before any suffix).
//
// v1.1.6 Part B: this used to budget 3 extra characters for an "hc-"
// prefix this function always added (see below) — now removed, so there's
// technically a little more DNS-length headroom available than before,
// left unclaimed here rather than bumping MAX_SEED_LENGTH too, since
// nothing in this version's brief asked for longer slugs and a length
// change is a separate, unrelated decision from a prefix removal.
 ...
 *
 * v1.1.6 Part B: NO LONGER prepends an "hc-" prefix. HeartCode's own root
 * domain already distinguishes its infrastructure from anything else on
 * Cloudflare Pages — the prefix was redundant, and a client's own
 * "happybirthday-zae8" reads cleaner than "hc-happybirthday-zae8" for a
 * link they're actually sharing. This ONLY affects project names created
 * from here forward: nothing in this function (or anywhere else this
 * version touches) reads, renames, or migrates any EXISTING
 * deployed_sites.cloudflare_project_name row — an already-live site's
 * Cloudflare project and URL are untouched, permanently, by construction
 * (this function is only ever called at the moment a NEW project is
 * created — see deployToCloudflarePages below).
 *
 * `seed` is still expected free of any prefix this function itself would
 * add — moot now that there's no prefix to double up, but callers (see
 * lib/finalizeDeployment.js) still separately strip pending.reference's
 * OWN leading "hc-" before calling this, for an unrelated reason: that
 * prefix belongs to the Paystack reference string, not to this function,
 * and needs stripping regardless of what this function does with its
 * seed.
 *
...
function buildProjectName(seed, { appendRandom = true } = {}) {
  ...
  return appendRandom ? `${cleanSeed}-${randomSuffix()}` : cleanSeed;
}
