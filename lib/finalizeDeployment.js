...
const { logEvent } = require('./activityEvents');
 ...
 *
 * v1.2.5: `source` (default `'webhook_or_callback'`) — pass `'recovery'`
 * ONLY from routes/adminRecovery.js's retry route. It changes nothing
 * about the deployment logic itself; it only tags the activity-feed
 * event a successful outcome logs (see lib/activityEvents.js), so an
 * admin-recovered payment shows up in the feed distinctly from the
 * normal live-webhook path rather than looking identical to it.
 */
async function finalizeDeployment(reference, { skipExpiryCheck = false, source = 'webhook_or_callback' } = {}) {
  const pool = getPool();
  ...
  try {
    ...
    // v1.2.5: logged inside the same transaction as everything below, so
    // an event never exists for a payment whose deployment then rolled
    // back (a crash between here and COMMIT undoes this insert too, same
    // as every other write in this function). Uses the same USD
    // back-calculation as the deployed_sites insert further down (see
    // that comment for why) rather than a simplified USD-only check, so
    // this event's amountUsd and the deployment_completed event's agree.
    const paymentAmountUsd = pending.charge_currency === 'USD'
    ...
    await logEvent(client, {
      ...
      title: `Payment received (${pending.charge_currency} ${Number(pending.charge_amount).toFixed(2)})`,
    ...
    // v1.2.5: same transaction as the insert above (see the payment_received
    // comment for why). websiteTypeRow can be null (a deployment whose type
    // was later deleted) — the title just omits the type name in that case
    // rather than failing the event.
    await logEvent(client, {
      eventType: 'deployment_completed',
      title: websiteTypeRow ? `${websiteTypeRow.name} site deployed` : 'Site deployed',
      detail: site.site_url,
      reference: site.reference,
      amountUsd: chargeAmountUsd,
      metadata: { websiteTypeId: pending.website_type_id, websiteTypeName: websiteTypeRow ? websiteTypeRow.name : null }
    });
    if (source === 'recovery') {
      await logEvent(client, {
        eventType: 'recovery_completed',
        title: websiteTypeRow ? `Recovered: ${websiteTypeRow.name} site` : 'Deployment recovered',
        detail: 'Recovered via admin retry after the original webhook did not complete it',
        reference: site.reference,
        amountUsd: chargeAmountUsd
      });
    }
    ...
    try {
      ...
      // v1.2.5: post-commit (this whole block runs after COMMIT above), so
      // uses the pool directly rather than the transaction's `client` —
      // there's no transaction left for this to be atomic with by this
      // point. Logged only on SUCCESS; a send failure falls straight to
      // the catch below without an email_sent event, which is exactly
      // right (nothing was actually sent).
      await logEvent(getPool(), {
        eventType: 'email_sent',
        title: sentCustom ? 'Confirmation email sent (custom template)' : 'Confirmation email sent',
        detail: pending.client_email,
        reference: pending.reference
      });
    } catch (err) {
