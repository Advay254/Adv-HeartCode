...
async function finalizeDeployment(reference) {
  ...
  try {
    ...
    const expectedAmountKobo = pending.amount_kes * 100;
    if (!txn || txn.status !== 'success' || txn.amount !== expectedAmountKobo) {
      await client.query('COMMIT');
    ...
    const insertResult = await client.query(
      `INSERT INTO deployed_sites (reference, website_type_id, client_email, site_url, cloudflare_project_name, amount_kes)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
      [pending.reference, pending.website_type_id, pending.client_email, deployResult.url, deployResult.projectName, pending.amount_kes]
    );
