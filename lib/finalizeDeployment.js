 ...
 * content still ships to every visitor's browser regardless (Cloudflare
 * Pages serves static files; there's no server-side enforcement without a
 * Worker doing real auth, which is out of scope here). It's obscurity:
 * enough to stop a site from being casually stumbled on by someone
 * browsing *.pages.dev or guessing a URL, not enough to stop anyone who
 * opens DevTools or views page source. This distinction matters for how
 * it gets described to clients — "password protected" oversells what
 * this actually does.
 */
...
async function finalizeDeployment(reference, { skipExpiryCheck = false } = {}) {
  ...
  try {
    ...
    // to the payment reference format, not to Cloudflare project naming,
    // and is unconditionally stripped here before use as a deploy seed.
    //
    // v1.1.6 Part B: this strip step matters MORE now than it used to,
    // not less. Before this version, buildProjectName() always prepended
    // its own separate "hc-" — stripping the reference's prefix here
    // existed only to avoid double-prefixing ("hc-hc-..."), since either
    // prefix alone would have been fine. Now that buildProjectName no
    // longer adds any prefix of its own, this IS the only thing standing
    // between a new deploy getting a clean "happybirthday-zae8" project
    // name (the actual goal of this version's change) versus an
    // accidental "hc-a1b2c3d4-zae8" — the literal "hc-" text sitting
    // inside pending.reference would otherwise flow straight through
    // slugify() into the final project name unchanged.
    ...
    if (deploySlugPattern) {
      ...
      if (resolvedSlug) {
        ...
        // v1.1.6 Part B: checked WITHOUT an "hc-" prefix now, matching
        // exactly what buildProjectName() actually produces for a new
        // deployment (see that function — it no longer adds one). An
        // existing pre-v1.1.6 row's project name still has the old
        // "hc-"-prefixed value stored (untouched, per this version's
        // known constraint), so it simply isn't a string this check
        // could ever match against going forward — which is correct:
        // "happybirthday-zae8" (new) and "hc-happybirthday-zae8" (old)
        // are two different, non-colliding Cloudflare project names as
        // far as Cloudflare itself is concerned, so there's no real
        // collision being missed here, only a stale one that could never
