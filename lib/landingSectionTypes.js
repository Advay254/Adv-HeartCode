...
// v1.1.5 Part B: replaces the fixed, non-CMS "website type teaser"
// section that used to be hardcoded directly in views/public/landing.ejs
// — brought into this system specifically to fix two real bugs at once:
// its heading said "business" (wrong for this product — HeartCode is
// personal, not business-facing) and neither its eyebrow text nor its
// heading was ever editable through the admin dashboard, unlike every
// other section here. Deliberately a SMALL content shape — just the copy
// around the cards, not the cards themselves. The actual category/type
// data the cards show is live DB state (which categories exist, which
// types are cheapest), not admin-authored content, so it's computed
// fresh at render time in routes/public.js's GET / handler rather than
// stored here — same reasoning DEFAULT_CONTENT.hero's comment gives for
// why THAT schema has no image field: this schema simply has no field
// for something that was never going to be admin-editable JSON in the
// first place.
const categoryTeaserSchema = z.object({
  eyebrow_text: z.string().trim().max(100).optional().default(''),
  heading: z.string().trim().min(1).max(200),
  highlighted_word: z.string().trim().max(100).optional().default('')
});
...
// v1.1.6 Part D: same reasoning as categoryTeaserSchema above (and the
// same reason DEFAULT_CONTENT.hero's comment gives) — deliberately a
// SMALL content shape, just the copy around the questions. The actual
// Q&A pairs are managed on their own dedicated admin page (mirroring
// Categories/Fields' list-management pattern — see routes/adminFaq.js)
// and live in their own `faq_entries` table, not as admin-authored JSON
// on this section's content — so, like category_teaser, this schema
// simply has no field for something that was never going to be
// section-content in the first place. Fetched fresh at render time in
// routes/public.js's GET / handler, same as category_teaser's live
// category/type data.
...
// v1.2.0: internal cross-linking between SEO landing pages (e.g. an
// "anniversary website" page linking to "birthday website") -- the same
// pattern competitors use for this. Deliberately plain { label, url }
// pairs rather than a reference to another seo_pages row by id: an admin
// may well want to link to /explore, a specific website type, or an
// external page just as easily as another SEO page, and a hardcoded id
// reference would break silently if that page were ever deleted (this
// way, a stale link is just a stale link, caught the same way any other
// footer_link_columns URL already is -- by an admin noticing, not by a
// foreign key). No image/icon field -- this is a plain text link list,
// closer in spirit to footerLinkColumnSchema above than to any
// image-bearing section type.
const relatedPageLinkSchema = z.object({
  label: z.string().trim().min(1).max(150),
  url: requiredUrlField
});
const relatedPagesSchema = z.object({
  heading: z.string().trim().min(1).max(200),
  links: z.array(relatedPageLinkSchema).max(20).optional().default([])
});
...
const SECTION_SCHEMAS = {
  ...
  footer: footerSchema,
  category_teaser: categoryTeaserSchema,
  faq: faqSchema,
  related_pages: relatedPagesSchema
};
...
const DEFAULT_CONTENT = {
  ...
  faq: {
    ...
    highlighted_word: 'actually ask'
  },
  related_pages: {
    heading: 'You might also be looking for',
    links: []
  }
};
