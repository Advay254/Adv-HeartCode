...
const DEFAULTS = {
  ...
  // v1.1.6 Part D: Organization structured data fields -- see
  // db/init.js's v1.1.6 migration comment for why these are separate
  // from og_image_url, and routes/public.js for how a blank value is
  // omitted from the rendered JSON-LD entirely rather than emitted empty.
  logo_url: '',
  contact_email: '',
  social_twitter_url: '',
  social_facebook_url: '',
  ...
  social_linkedin_url: '',
  // v1.1.9 Part B: master on/off switch for whether a specific type's
  // price renders on /explore's cards and the /build form header. Off
  // ('false') by default -- see routes/public.js's explore/explore-category/
  // build handlers (which pass this straight through as part of
  // siteSettings, no separate query needed) and the matching views, which
  // wrap their price blocks in a check against this exact string value.
  // Deliberately does NOT gate checkout (websiteType.chargeDisplay is a
  // separate field entirely -- see routes/public.js's resolveChargeForCheckout)
  // or the landing page's category-teaser "From $X" line (see
  // views/partials/landing-sections/category-teaser.ejs, which never reads
  // this setting at all) -- both are real commitments/teasers, not the
  // "type gallery + form" browsing surface this toggle is scoped to.
  show_type_prices_early: 'false'
};
